...
class MySexyVariables:
    ...
    bitwise_calls_list = [" AND", " OR", " XOR", " NOT", " RIGHT SHIFT", " LEFT SHIFT"]
...
class Input:
    ...
    def get_string_input():
        ...
        return console.input(
            " [bright_black]_______________________________________________[/bright_black]"
            + "[bright_black]\n  _ [/bright_black]"
            + "[red1]"
            + MySexyVariables.curdir
            + " [/red1]"
            + "[bright_black]\n (__ [/bright_black]"
            + "[purple]"
            + MySexyVariables.user
            + "[/purple]"
            + "[bright_black] __: [/bright_black]"
        )
...
def Bitwise_is_hard_af():
    ...
    main_logo.logo()
    print(
        " [bright_black]Bitwise Sublevels: Hell's Calculator\n bitwise number or exit[/bright_black]"
    )
    while True:
        calls.bitwise_list()
        ...
        if operation.upper() == "NOT":
            ...
            print(
                " " + bin(result)[2:].zfill(max(len_num1, len_num2)) + " binary result"
            )
            print(f" Result: {result}")
            continue
...
def convert_to_codes(input_value):
    ...
    for char in input_value:
        ...
        print(
            f"Character: {char} ASCII: {ascii_code} Binary: {binary_code} Unicode: {unicode_code} Unicode Decimal: {ascii_code} Hexadecimal: {hexadecimal_code} Octal: {octal_code}"
        )
...
def string_conversions():
    while True:
        print(
            " [bright_black]Input a string to convert\n or no string to exit.[/bright_black]"
        )
